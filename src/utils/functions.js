...
export async function createList() {
  let {
      data: {response: covidSum},
    } = await axios.get('https://covid-193.p.rapidapi.com/statistics', {
      headers: {
        'x-rapidapi-host': 'covid-193.p.rapidapi.com',
        'x-rapidapi-key': '5f393b737amsh9482f71aed23c11p122977jsnd46a7a6638af',
      },
    }),
    {data: countries} = await axios.get(
      'https://ajayakv-rest-countries-v1.p.rapidapi.com/rest/v1/all',
      {
        headers: {
          'x-rapidapi-host': 'ajayakv-rest-countries-v1.p.rapidapi.com',
          'x-rapidapi-key':
            '5f393b737amsh9482f71aed23c11p122977jsnd46a7a6638af',
        },
      },
    );
...
function convertToData(covidSum, countries) {
  ...
  for (let conIndex = 0; conIndex < countriesLen; conIndex++) {
    for (let covIndex = 0; covIndex < covidLen; covIndex++) {
      if (
        countries[conIndex].name.trim() === covidSum[covIndex].country.trim()
      ) {
        let dataObj = {},
        ...
        dataObj = {...currentCov};
        dataObj.code = currentCon.alpha2Code;
  ...
  return [
    ...
    responseData
      .map((item, index) => [+item.cases.total, {...item, listIndex: index}])
      .sort(([a], [b]) => b - a)
      .map(([, item]) => item),
    responseData
      .map((item, index) => [
